...
IGenericOperation<Instructor> instructorOperation = new InstructorOperation();
IGenericOperation<Course> courseOperation = new CourseOperation();
IGenericOperation<Student> studentOperation = new StudentOperation();
IGenericOperation<FeedBackCourse> FeedBackOperation = new FeedBackCourseOperation();
var option = "";
while (option != "ex")
{
    ...
    if (option == "1")
    {
        ...
        if (Operation == "1")
        {
            ...
            var isIdExist = courseOperation.GetAll().FirstOrDefault(c => c.CourseId == courseId);
            var isCourseName = courseOperation.GetAll().FirstOrDefault(c => c.CourseName.ToLower() == courseName.ToLower());
            if (isIdExist != null)
            {
                Console.WriteLine($"This Course Id already exist {courseId}");
                Console.WriteLine("---------------------------------");
            }
            else if (isCourseName != null)
            {
    ...
    else if (option == "2")
    {
        ...
        if (operation == "1")
        {
            ...
            var isIdExist = instructorOperation.GetAll().FirstOrDefault(c => c.InstructorId == Id);
            var isInstructorName = instructorOperation.GetAll().FirstOrDefault(c => c.InstructorName.ToLower() == Name.ToLower());
            if (isIdExist != null)
            {
                Console.WriteLine($"This Id already exist {Id}");
                Console.WriteLine("---------------------------------");
            }
            else if (isInstructorName != null)
            {
    ...
    else if (option == "3")
    {
        ...
        if (operation == "1")
        {
            ...
            while (addLoop != "no")
            {
                ...
                var courseId = int.Parse(Console.ReadLine());
                student.Courses.Add(new StudentCourse { CourseId = courseId, StudentId = Id });
